 ...
 var url = baseUrl + 'web/web/insertMagentoCart/'+sess_id;
    ...
    doAjaxCall(url, data, true, function (html) {
        if (html != 0) {
			...
			//var last_id=1;
			window.location.href = 'magento_checkout_cart.html?touchId=' + touchId + '&location=' + location + '&last_id=' + html+'&productId=' + productId+ '&stock=' +stock;
...
//var last_id = getUrlVars()['last_id'];
var data='';
var url = baseUrl + 'web/web/magentocartinfo/' + userSiteId + '/' + location + '/' + last_id+ '/' + productId ;
...
function updatePrices() {
    ...
    $('tr .otherCharges').each(function () {
        ...
        var quantity = parseInt(previousTr.find('.quantity').html());
...
if($('#cash').val()=="" && $('#paypal').val()=="")
	...
	//alert(adminEmail);
	//var form_id=$("#merchandisecartform");
    var url = baseUrl + 'web/web/doDirectMagentoPayment/'+ merch_id + '/' + cartId+'/'+ payment_type ;
    // just to check weather form is valid or not
    if ($('#ordertablePrice').length > 0) {
        //alert("Placing order");
        // perform your validation here
